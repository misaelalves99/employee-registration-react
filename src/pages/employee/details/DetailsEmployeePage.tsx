...
export default function DetailsEmployeePage({ id }: Props) {
  const { employees } = useEmployee(); // hook do contexto
  ...
  useEffect(() => {
    ...
    const foundEmployee = employees.find(emp => emp.id === numericId);
    if (!foundEmployee) {
      setNotFound(true);
      return;
    }
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.buttonGroup}>
        <button className={styles.btnSecondary} onClick={() => navigate('/employee')} type="button">
          Voltar
        </button>
        <button className={styles.btnPrimary} onClick={() => navigate(`/employee/edit/${employee.id}`)} type="button">
          Editar
        </button>
      </div>
    </div>
